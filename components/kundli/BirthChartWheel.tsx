      ...
      <svg viewBox="0 0 400 400" width={size} height={size}>
        ...
        <defs>
          <radialGradient id="bgGlow" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#1a0a00" stopOpacity="0.6" />
            <stop offset="100%" stopColor="#000000" stopOpacity="1" />
          </radialGradient>
          <radialGradient id="innerGlow" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#0d0500" stopOpacity="0.8" />
            <stop offset="100%" stopColor="#000" stopOpacity="1" />
          </radialGradient>
        </defs>
        <circle cx={CX} cy={CY} r={196} fill="url(#bgGlow)" />
        <circle cx={CX} cy={CY} r={196} fill="none" stroke="#C4922A" strokeWidth="0.8" opacity="0.5" />
        ...
        {Array.from({ length: 12 }, (_, i) => {
          ...
          return (
            <g key={`z${i}`}>
              <path d={wedge(h, 158, 190)} fill={i % 2 ? "#0a0500" : "#050200"} stroke="#C4922A" strokeWidth="0.5" opacity="0.9" />
              <text x={f(c.x)} y={f(c.y)} textAnchor="middle" dominantBaseline="middle" fill="#C4922A" fontSize="13" fontFamily="serif">{ZODIAC_SYMS[signIdx]}</text>
            </g>
          );
        })}
        ...
        {Array.from({ length: 12 }, (_, i) => {
          ...
          return (
            <g key={`h${i}`}>
              <path d={wedge(h, 118, 158)} fill={i % 2 ? "#070707" : "#040404"} stroke="#1a1a1a" strokeWidth="0.4" />
              <text x={f(c.x)} y={f(c.y)} textAnchor="middle" dominantBaseline="middle" fill="#444" fontSize="9" fontFamily="Times New Roman, serif">{h}</text>
            </g>
          );
        })}
        ...
        {Array.from({ length: 12 }, (_, i) => {
          ...
          return <line key={`d${i}`} x1={f(inner.x)} y1={f(inner.y)} x2={f(outer.x)} y2={f(outer.y)} stroke="#C4922A" strokeWidth="0.4" opacity="0.3" />;
        })}

        {/* Ring borders */}
        <circle cx={CX} cy={CY} r={158} fill="none" stroke="#222" strokeWidth="0.6" />
        <circle cx={CX} cy={CY} r={118} fill="none" stroke="#1a1a1a" strokeWidth="0.6" />
        ...
        {Object.entries(byHouse).map(([hs, hPlanets]) => {
          ...
          return hPlanets.map((planet, pIdx) => {
            ...
            const col = PLANET_COLORS[planet.name] ?? "#fff";
            return (
              <g key={`pl-${planet.name}`}>
                <circle cx={f(pos.x)} cy={f(pos.y)} r="12" fill="#111" stroke={col} strokeWidth="0.8" opacity="0.85" />
        ...
        {/* Center */}
        <circle cx={CX} cy={CY} r={50} fill="#040404" stroke="#C4922A" strokeWidth="0.5" opacity="0.5" />
        {/* Inner mandala decoration */}
        {[40, 30, 20].map(r => (
          <circle key={r} cx={CX} cy={CY} r={r} fill="none" stroke="#C4922A" strokeWidth="0.2" opacity={0.15 + (40-r)*0.01} />
        ))}
        <text x={CX} y={CY - 10} textAnchor="middle" fill="#C4922A" fontSize="7" fontFamily="Times New Roman, serif" letterSpacing="2">LAGNA</text>
        <text x={CX} y={CY + 4}  textAnchor="middle" fill="#ffffff" fontSize="11" fontFamily="Times New Roman, serif" fontWeight="bold">{ascendant.slice(0,3).toUpperCase()}</text>
        <text x={CX} y={CY + 17} textAnchor="middle" fill="#555" fontSize="7" fontFamily="Times New Roman, serif">☽ {moonSign.slice(0,3).toUpperCase()}</text>
      </svg>
    </div>
  );
}
